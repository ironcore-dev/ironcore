...
// List lists all VolumePools in the indexer.
func (s *volumePoolLister) List(selector labels.Selector) (ret []*storage.VolumePool, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*storage.VolumePool))
	})
	return ret, err
}

// Get retrieves the VolumePool from the index for a given name.
func (s *volumePoolLister) Get(name string) (*storage.VolumePool, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(storage.Resource("volumepool"), name)
	}
	return obj.(*storage.VolumePool), nil
}
