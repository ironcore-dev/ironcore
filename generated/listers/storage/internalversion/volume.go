...
// List lists all Volumes in the indexer.
func (s *volumeLister) List(selector labels.Selector) (ret []*storage.Volume, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*storage.Volume))
	})
	return ret, err
}
...
// List lists all Volumes in the indexer for a given namespace.
func (s volumeNamespaceLister) List(selector labels.Selector) (ret []*storage.Volume, err error) {
...
// Get retrieves the Volume from the indexer for a given namespace and name.
func (s volumeNamespaceLister) Get(name string) (*storage.Volume, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(storage.Resource("volume"), name)
	}
	return obj.(*storage.Volume), nil
}
