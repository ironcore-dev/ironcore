...
type NetworkLister interface {
	...
	List(selector labels.Selector) (ret []*networking.Network, err error)
...
// List lists all Networks in the indexer.
func (s *networkLister) List(selector labels.Selector) (ret []*networking.Network, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*networking.Network))
	})
	return ret, err
}
...
type NetworkNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*networking.Network, error)
...
// List lists all Networks in the indexer for a given namespace.
func (s networkNamespaceLister) List(selector labels.Selector) (ret []*networking.Network, err error) {
...
// Get retrieves the Network from the indexer for a given namespace and name.
func (s networkNamespaceLister) Get(name string) (*networking.Network, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(networking.Resource("network"), name)
	}
	return obj.(*networking.Network), nil
}
