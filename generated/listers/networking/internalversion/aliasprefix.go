...
type AliasPrefixLister interface {
	...
	List(selector labels.Selector) (ret []*networking.AliasPrefix, err error)
...
// List lists all AliasPrefixes in the indexer.
func (s *aliasPrefixLister) List(selector labels.Selector) (ret []*networking.AliasPrefix, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*networking.AliasPrefix))
	})
	return ret, err
}
...
type AliasPrefixNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*networking.AliasPrefix, error)
...
// List lists all AliasPrefixes in the indexer for a given namespace.
func (s aliasPrefixNamespaceLister) List(selector labels.Selector) (ret []*networking.AliasPrefix, err error) {
...
// Get retrieves the AliasPrefix from the indexer for a given namespace and name.
func (s aliasPrefixNamespaceLister) Get(name string) (*networking.AliasPrefix, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(networking.Resource("aliasprefix"), name)
	}
	return obj.(*networking.AliasPrefix), nil
}
