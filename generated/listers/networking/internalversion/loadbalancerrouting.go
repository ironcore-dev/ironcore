...
type LoadBalancerRoutingLister interface {
	...
	List(selector labels.Selector) (ret []*networking.LoadBalancerRouting, err error)
...
// List lists all LoadBalancerRoutings in the indexer.
func (s *loadBalancerRoutingLister) List(selector labels.Selector) (ret []*networking.LoadBalancerRouting, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*networking.LoadBalancerRouting))
	})
	return ret, err
}
...
type LoadBalancerRoutingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*networking.LoadBalancerRouting, error)
...
// List lists all LoadBalancerRoutings in the indexer for a given namespace.
func (s loadBalancerRoutingNamespaceLister) List(selector labels.Selector) (ret []*networking.LoadBalancerRouting, err error) {
...
// Get retrieves the LoadBalancerRouting from the indexer for a given namespace and name.
func (s loadBalancerRoutingNamespaceLister) Get(name string) (*networking.LoadBalancerRouting, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(networking.Resource("loadbalancerrouting"), name)
	}
	return obj.(*networking.LoadBalancerRouting), nil
}
