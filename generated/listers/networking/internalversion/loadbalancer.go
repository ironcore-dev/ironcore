...
type LoadBalancerLister interface {
	...
	List(selector labels.Selector) (ret []*networking.LoadBalancer, err error)
...
// List lists all LoadBalancers in the indexer.
func (s *loadBalancerLister) List(selector labels.Selector) (ret []*networking.LoadBalancer, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*networking.LoadBalancer))
	})
	return ret, err
}
...
type LoadBalancerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*networking.LoadBalancer, error)
...
// List lists all LoadBalancers in the indexer for a given namespace.
func (s loadBalancerNamespaceLister) List(selector labels.Selector) (ret []*networking.LoadBalancer, err error) {
...
// Get retrieves the LoadBalancer from the indexer for a given namespace and name.
func (s loadBalancerNamespaceLister) Get(name string) (*networking.LoadBalancer, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(networking.Resource("loadbalancer"), name)
	}
	return obj.(*networking.LoadBalancer), nil
}
