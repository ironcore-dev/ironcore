...
// List lists all PrefixAllocations in the indexer.
func (s *prefixAllocationLister) List(selector labels.Selector) (ret []*ipam.PrefixAllocation, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*ipam.PrefixAllocation))
	})
	return ret, err
}
...
// List lists all PrefixAllocations in the indexer for a given namespace.
func (s prefixAllocationNamespaceLister) List(selector labels.Selector) (ret []*ipam.PrefixAllocation, err error) {
...
// Get retrieves the PrefixAllocation from the indexer for a given namespace and name.
func (s prefixAllocationNamespaceLister) Get(name string) (*ipam.PrefixAllocation, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(ipam.Resource("prefixallocation"), name)
	}
	return obj.(*ipam.PrefixAllocation), nil
}
