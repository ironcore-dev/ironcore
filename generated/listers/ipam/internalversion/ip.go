...
type IPLister interface {
	...
	List(selector labels.Selector) (ret []*ipam.IP, err error)
...
// List lists all IPs in the indexer.
func (s *iPLister) List(selector labels.Selector) (ret []*ipam.IP, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*ipam.IP))
	})
	return ret, err
}
...
// List lists all IPs in the indexer for a given namespace.
func (s iPNamespaceLister) List(selector labels.Selector) (ret []*ipam.IP, err error) {
...
// Get retrieves the IP from the indexer for a given namespace and name.
func (s iPNamespaceLister) Get(name string) (*ipam.IP, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(ipam.Resource("ip"), name)
	}
	return obj.(*ipam.IP), nil
}
