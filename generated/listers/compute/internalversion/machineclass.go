...
type MachineClassLister interface {
	...
	List(selector labels.Selector) (ret []*compute.MachineClass, err error)
...
// List lists all MachineClasses in the indexer.
func (s *machineClassLister) List(selector labels.Selector) (ret []*compute.MachineClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*compute.MachineClass))
	})
	return ret, err
}
...
type MachineClassNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*compute.MachineClass, error)
...
// List lists all MachineClasses in the indexer for a given namespace.
func (s machineClassNamespaceLister) List(selector labels.Selector) (ret []*compute.MachineClass, err error) {
...
// Get retrieves the MachineClass from the indexer for a given namespace and name.
func (s machineClassNamespaceLister) Get(name string) (*compute.MachineClass, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(compute.Resource("machineclass"), name)
	}
	return obj.(*compute.MachineClass), nil
}
