...
type MachineLister interface {
	...
	List(selector labels.Selector) (ret []*compute.Machine, err error)
...
// List lists all Machines in the indexer.
func (s *machineLister) List(selector labels.Selector) (ret []*compute.Machine, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*compute.Machine))
	})
	return ret, err
}
...
type MachineNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*compute.Machine, error)
...
// List lists all Machines in the indexer for a given namespace.
func (s machineNamespaceLister) List(selector labels.Selector) (ret []*compute.Machine, err error) {
...
// Get retrieves the Machine from the indexer for a given namespace and name.
func (s machineNamespaceLister) Get(name string) (*compute.Machine, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(compute.Resource("machine"), name)
	}
	return obj.(*compute.Machine), nil
}
