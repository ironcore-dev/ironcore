...
type NetworkInterfaceLister interface {
	...
	List(selector labels.Selector) (ret []*compute.NetworkInterface, err error)
...
// List lists all NetworkInterfaces in the indexer.
func (s *networkInterfaceLister) List(selector labels.Selector) (ret []*compute.NetworkInterface, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*compute.NetworkInterface))
	})
	return ret, err
}
...
type NetworkInterfaceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*compute.NetworkInterface, error)
...
// List lists all NetworkInterfaces in the indexer for a given namespace.
func (s networkInterfaceNamespaceLister) List(selector labels.Selector) (ret []*compute.NetworkInterface, err error) {
...
// Get retrieves the NetworkInterface from the indexer for a given namespace and name.
func (s networkInterfaceNamespaceLister) Get(name string) (*compute.NetworkInterface, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(compute.Resource("networkinterface"), name)
	}
	return obj.(*compute.NetworkInterface), nil
}
