...
type StoragePoolInformer interface {
	...
	Lister() internalversion.StoragePoolLister
}
...
func NewStoragePoolInformer(client clientsetinternalversion.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredStoragePoolInformer(client clientsetinternalversion.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Storage().StoragePools().List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Storage().StoragePools().Watch(context.TODO(), options)
			},
		},
		&storage.StoragePool{},
		resyncPeriod,
...
func (f *storagePoolInformer) defaultInformer(client clientsetinternalversion.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *storagePoolInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&storage.StoragePool{}, f.defaultInformer)
}

func (f *storagePoolInformer) Lister() internalversion.StoragePoolLister {
	return internalversion.NewStoragePoolLister(f.Informer().GetIndexer())
}
