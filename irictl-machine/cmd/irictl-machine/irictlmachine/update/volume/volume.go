...
func Run(ctx context.Context, streams clicommon.Streams, client iri.MachineRuntimeClient, opts Options) error {
	...
	if _, err := client.UpdateVolume(ctx, &iri.UpdateVolumeRequest{
		MachineId: opts.MachineID,
		Volume:    volume,
	}); err != nil {
		return fmt.Errorf("error updating volume %s in machine %s: %w", volume.Name, opts.MachineID, err)
	}

	_, _ = fmt.Fprintf(streams.Out, "Updated volume %s in machine %s\n", volume.Name, opts.MachineID)
	return nil
}
