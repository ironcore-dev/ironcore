...
func createChannels(opts StreamsOptions) []wsstream.ChannelType {
...
// createWebSocketStreams returns a streams containing the websocket connection and
// streams needed to perform an exec or an attach.
func createWebSocketStreams(req *http.Request, w http.ResponseWriter, opts StreamsOptions) (*streams, bool) {
	channelTypes := createChannels(opts)
	...
	negotiatedProtocol, channels, err := conn.Open(responsewriter.GetOriginal(w), req)
	if err != nil {
	...
	switch {
	case opts.Stdout:
		_, _ = channels[stdoutChannel].Write([]byte{})
	case opts.Stderr:
		_, _ = channels[stderrChannel].Write([]byte{})
	default:
		_, _ = channels[errorChannel].Write([]byte{})
	}

	s := &streams{
		conn:         conn,
		stdinStream:  channels[stdinChannel],
		stdoutStream: channels[stdoutChannel],
		stderrStream: channels[stderrChannel],
		resizeStream: channels[resizeChannel],
	}

	switch negotiatedProtocol {
	case v4BinaryWebsocketProtocol, v4Base64WebsocketProtocol:
		s.writeStatus = v4WriteStatusFunc(channels[errorChannel])
	default:
		s.writeStatus = v1WriteStatusFunc(channels[errorChannel])
	}
