...
func IsClusterPrefixReady(prefix *networkv1alpha1.ClusterPrefix) bool {
	return conditionutils.MustFindSliceStatus(prefix.Status.Conditions, string(networkv1alpha1.ClusterPrefixReady)) == corev1.ConditionTrue
}
...
//+kubebuilder:rbac:groups=network.onmetal.de,resources=clusterprefixes,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=network.onmetal.de,resources=clusterprefixes/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=network.onmetal.de,resources=clusterprefixes/finalizers,verbs=update
//+kubebuilder:rbac:groups=network.onmetal.de,resources=clusterprefixallocations,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=network.onmetal.de,resources=clusterprefixallocations/status,verbs=get;update;patch
...
func (r *ClusterPrefixReconciler) reconcileExists(ctx context.Context, log logr.Logger, prefix *networkv1alpha1.ClusterPrefix) (ctrl.Result, error) {
...
func (r *ClusterPrefixReconciler) delete(ctx context.Context, log logr.Logger, prefix *networkv1alpha1.ClusterPrefix) (ctrl.Result, error) {
	...
	clusterList := &networkv1alpha1.ClusterPrefixAllocationList{}
...
func (r *ClusterPrefixReconciler) patchReadiness(ctx context.Context, prefix *networkv1alpha1.ClusterPrefix, readyCond networkv1alpha1.ClusterPrefixCondition) error {
	base := prefix.DeepCopy()
	conditionutils.MustUpdateSlice(&prefix.Status.Conditions, string(networkv1alpha1.ClusterPrefixReady),
...
func (r *ClusterPrefixReconciler) assignValues(ctx context.Context, prefix *networkv1alpha1.ClusterPrefix, res PrefixAllocationer) error {
	base := prefix.DeepCopy()
	...
	if parentRef.Kind != networkv1alpha1.ClusterPrefixKind {
		return fmt.Errorf("invalid parent ref kind %q, expected %q", parentRef.Kind, networkv1alpha1.ClusterPrefixKind)
	}
...
func (r *ClusterPrefixReconciler) allocateSelf(ctx context.Context, log logr.Logger, prefix *networkv1alpha1.ClusterPrefix) (bool, error) {
	switch {
	...
	case IsRootClusterPrefix(prefix):
		...
		if err := r.patchReadiness(ctx, prefix, networkv1alpha1.ClusterPrefixCondition{
...
func (r *ClusterPrefixReconciler) processAllocations(ctx context.Context, log logr.Logger, prefix *networkv1alpha1.ClusterPrefix) (available, reserved []commonv1alpha1.IPPrefix, err error) {
	...
	var newClusterAllocations []networkv1alpha1.ClusterPrefixAllocation
	...
	var newAllocations []networkv1alpha1.PrefixAllocation
...
func (r *ClusterPrefixReconciler) processAllocation(ctx context.Context, log logr.Logger, available *netaddr.IPSet, allocation *networkv1alpha1.PrefixAllocation) (*netaddr.IPSet, error) {
...
func (r *ClusterPrefixReconciler) patchClusterAllocationReadiness(
	ctx context.Context,
	allocation *networkv1alpha1.ClusterPrefixAllocation,
	prefix netaddr.IPPrefix,
	readyCond networkv1alpha1.ClusterPrefixAllocationCondition,
) error {
	base := allocation.DeepCopy()
	allocation.Status.Prefix = commonv1alpha1.IPPrefix{IPPrefix: prefix}
	conditionutils.MustUpdateSlice(&allocation.Status.Conditions, string(networkv1alpha1.ClusterPrefixAllocationReady),
		conditionutils.UpdateStatus(readyCond.Status),
		conditionutils.UpdateReason(readyCond.Reason),
		conditionutils.UpdateMessage(readyCond.Message),
	)
...
func (r *ClusterPrefixReconciler) processClusterAllocation(ctx context.Context, log logr.Logger, available *netaddr.IPSet, allocation *networkv1alpha1.ClusterPrefixAllocation) (*netaddr.IPSet, error) {
	log = log.WithValues("AllocationKey", client.ObjectKeyFromObject(allocation))
	prefix, _, newAvailableSet, ok := RemoveFreePrefixForRequest(available, networkv1alpha1.PrefixAllocationRequest{
	...
	case !ok:
		log.V(1).Info("Marking non-allocatable allocation as pending")
		if err := r.patchClusterAllocationReadiness(ctx, allocation, netaddr.IPPrefix{}, networkv1alpha1.ClusterPrefixAllocationCondition{
			Status:  corev1.ConditionFalse,
			Reason:  networkv1alpha1.ClusterPrefixAllocationReadyReasonPending,
			Message: "Could not allocate request.",
		}); err != nil {
			return available, fmt.Errorf("could not mark allocation as pending")
		}
	...
	default:
		log.V(1).Info("Marking allocation as allocated")
		if err := r.patchClusterAllocationReadiness(ctx, allocation, prefix, networkv1alpha1.ClusterPrefixAllocationCondition{
			Status:  corev1.ConditionTrue,
			Reason:  networkv1alpha1.ClusterPrefixAllocationReadyReasonSucceeded,
			Message: "The request has been successfully allocated.",
		}); err != nil {
			return available, fmt.Errorf("error marking allocation as ready: %w", err)
		}
...
func (r *ClusterPrefixReconciler) reconcile(ctx context.Context, log logr.Logger, prefix *networkv1alpha1.ClusterPrefix) (ctrl.Result, error) {
	if !controllerutil.ContainsFinalizer(prefix, ClusterPrefixFinalizer) {
		base := prefix.DeepCopy()
...
func (r *ClusterPrefixReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &networkv1alpha1.ClusterPrefixAllocation{}, clusterPrefixAllocationSpecPrefixRefNameField, func(obj client.Object) []string {
		prefix := obj.(*networkv1alpha1.ClusterPrefixAllocation)
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &networkv1alpha1.PrefixAllocation{}, prefixAllocationSpecPrefixRefIfKindIsClusterPrefixNameField, func(obj client.Object) []string {
		prefix := obj.(*networkv1alpha1.PrefixAllocation)
		prefixRef := prefix.Spec.PrefixRef
		if prefixRef == nil || prefixRef.Kind != networkv1alpha1.ClusterPrefixKind {
			return nil
		}
		return []string{prefixRef.Name}
	}); err != nil {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.ClusterPrefix{}).
		Owns(&networkv1alpha1.ClusterPrefixAllocation{}).
		Watches(
			&source.Kind{Type: &networkv1alpha1.PrefixAllocation{}},
			r.enqueueByAllocationPrefixRef(),
		).
		Watches(
			&source.Kind{Type: &networkv1alpha1.ClusterPrefixAllocation{}},
...
func (r *ClusterPrefixReconciler) enqueueByClusterAllocationPrefixRef() handler.EventHandler {
	return handler.EnqueueRequestsFromMapFunc(func(obj client.Object) []ctrl.Request {
		allocation := obj.(*networkv1alpha1.ClusterPrefixAllocation)
		prefixRef := allocation.Spec.PrefixRef
		if prefixRef == nil {
			return nil
		}
...
func (p *ClusterPrefix) Object() client.Object {
	return (*networkv1alpha1.ClusterPrefix)(p)
}
...
func (p *ClusterPrefix) PrefixSelector() *networkv1alpha1.PrefixSelector {
	...
	return &networkv1alpha1.PrefixSelector{
		Kind:          networkv1alpha1.ClusterPrefixKind,
