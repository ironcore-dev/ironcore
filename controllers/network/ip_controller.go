...
func IsIPReady(ip *networkv1alpha1.IP) bool {
	return conditionutils.MustFindSliceStatus(ip.Status.Conditions, string(networkv1alpha1.IPReady)) == corev1.ConditionTrue
}
...
//+kubebuilder:rbac:groups=network.onmetal.de,resources=ips,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=network.onmetal.de,resources=ips/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=network.onmetal.de,resources=ips/finalizers,verbs=update
...
func (r *IPReconciler) reconcileExists(ctx context.Context, log logr.Logger, ip *networkv1alpha1.IP) (ctrl.Result, error) {
...
func (r *IPReconciler) delete(ctx context.Context, log logr.Logger, ip *networkv1alpha1.IP) (ctrl.Result, error) {
	return ctrl.Result{}, nil
}

func (r *IPReconciler) patchReadiness(ctx context.Context, ip *networkv1alpha1.IP, readyCond networkv1alpha1.IPCondition) error {
	base := ip.DeepCopy()
	conditionutils.MustUpdateSlice(&ip.Status.Conditions, string(networkv1alpha1.IPReady),
...
func (r *IPReconciler) patchAssignment(ctx context.Context, ip *networkv1alpha1.IP, res PrefixAllocationer) error {
	base := ip.DeepCopy()
...
func (r *IPReconciler) reconcile(ctx context.Context, log logr.Logger, ip *networkv1alpha1.IP) (ctrl.Result, error) {
	switch {
	case IsIPReady(ip):
	...
	default:
		...
		switch {
		case err != nil:
			if err := r.patchReadiness(ctx, ip, networkv1alpha1.IPCondition{
		...
		case res.ReadyState() == PrefixAllocationSucceeded:
			...
			if err := r.patchReadiness(ctx, ip, networkv1alpha1.IPCondition{
				...
				Reason:  networkv1alpha1.IPReadyReasonAllocated,
		...
		default:
			...
			if err := r.patchReadiness(ctx, ip, networkv1alpha1.IPCondition{
				...
				Reason:  networkv1alpha1.IPReadyReasonPending,
