...
func IsClusterPrefixAllocationInTerminalState(allocation *networkv1alpha1.ClusterPrefixAllocation) bool {
	...
	conditionutils.MustFindSlice(allocation.Status.Conditions, string(networkv1alpha1.ClusterPrefixAllocationReady), readyCond)
	switch {
	case readyCond.Status == corev1.ConditionFalse && readyCond.Reason == networkv1alpha1.ClusterPrefixAllocationReadyReasonFailed:
		return true
...
//+kubebuilder:rbac:groups=network.onmetal.de,resources=clusterprefixallocations,verbs=get;list;watch;update;patch
...
func (r *ClusterPrefixAllocationSchedulerReconciler) reconcileExists(ctx context.Context, log logr.Logger, allocation *networkv1alpha1.ClusterPrefixAllocation) (ctrl.Result, error) {
...
func (r *ClusterPrefixAllocationSchedulerReconciler) patchReadiness(ctx context.Context, allocation *networkv1alpha1.ClusterPrefixAllocation, readyCond networkv1alpha1.ClusterPrefixAllocationCondition) error {
...
func (r *ClusterPrefixAllocationSchedulerReconciler) prefixRefForAllocation(ctx context.Context, allocation *networkv1alpha1.ClusterPrefixAllocation) (string, error) {
	...
	for _, clusterPrefix := range list.Items {
		...
		if CanClusterPrefixFitRequest(&clusterPrefix, networkv1alpha1.PrefixAllocationRequest{
...
func (r *ClusterPrefixAllocationSchedulerReconciler) reconcile(ctx context.Context, log logr.Logger, allocation *networkv1alpha1.ClusterPrefixAllocation) (ctrl.Result, error) {
	...
	if ref == "" {
		...
		if err := r.patchReadiness(ctx, allocation, networkv1alpha1.ClusterPrefixAllocationCondition{
...
func (r *ClusterPrefixAllocationSchedulerReconciler) SetupWithManager(mgr manager.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &networkv1alpha1.ClusterPrefixAllocation{}, clusterPrefixAllocationSpecIfUnscheduledPrefixField, func(obj client.Object) []string {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&networkv1alpha1.ClusterPrefixAllocation{}).
		Watches(
