...
import (
	...
	terminalhttp "github.com/onmetal/onmetal-api/machinepoollet/terminal/http"
...
func (s *Server) registerComputeRoutes(r *mux.Router) {
	r.Methods(http.MethodGet, http.MethodPost).Path("/namespaces/{namespace}/machines/{name}/exec").HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		streamOpts := &terminalhttp.Options{
			Stdin:  true,
			Stdout: true,
			TTY:    true,
		}
		terminalhttp.Serve(w, req, term, streamOpts, s.streamIdleTimeout, s.streamCreationTimeout, supportedStreamProtocols)
	})
}
