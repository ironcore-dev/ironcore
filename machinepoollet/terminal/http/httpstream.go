...
// Validate validates whether the given options are valid.
func (o *Options) Validate() error {
	if o.TTY && o.Stderr {
		return fmt.Errorf("cannot specify exec with tty and stderr as its bypassing stderr")
	}
...
// OptionsFromRequest parses Options from the Request.
func OptionsFromRequest(req *http.Request) (*Options, error) {
...
func createHTTPStreamStreams(req *http.Request, w http.ResponseWriter, opts *Options, supportedStreamProtocols []string, idleTimeout, streamCreationTimeout time.Duration) (*context, bool) {
	...
	case "":
		log.V(4).Info("Client did not request protocol negotiation. Falling back", "protocol", remotecommandconsts.StreamProtocolV1Name)
		fallthrough
